...
<section class="max-w-screen-lg min-h-screen mx-auto flex justify-center items-center pt-44 pb-20 px-4 lg:px-12 gap-4">
  <div class="w-full p-10 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
    ...
    <div class="mt-12">
      <form action="{{route('kajur.periode.update', $periode->id)}}" method="post" class=" w-full grid grid-cols-12 gap-4">
        @csrf
        @method('PUT')
        <div class="mb-4 col-span-12">
          ...
          <input id="nama_periode" name="name" placeholder="nama periode"
            class="block w-full xl:p-4 p-3 text-gray-900 border border-gray-300 rounded-md bg-gray-50 xl:text-sm text-xs " value="{{ $periode->name }}" />
        </div>
        <div class="mb-4 col-span-12">
          ...
          <textarea id="deksripsi_periode" name="deskripsi" placeholder="deksripsi periode"
            class="block w-full xl:p-4 p-3 text-gray-900 border border-gray-300 rounded-md bg-gray-50 xl:text-sm text-xs " >{{ $periode->deskripsi }}</textarea>
        </div>
        <div class="mb-4 col-span-6">
          ...
          <input type="date" max="100" id="tanggal_mulai" name="tanggal_mulai" placeholder="tanggal mulai"
            class="block w-full xl:p-4 p-3 text-gray-900 border border-gray-300 rounded-md bg-gray-50 xl:text-sm text-xs " value="{{ $periode->tgl_mulai }}"/>
        </div>
        <div class="mb-4 col-span-6">
          ...
          <input type="date" id="tanggal_berakhir" name="tanggal_berakhir" placeholder="tanggal berakhir"
            class="block w-full xl:p-4 p-3 text-gray-900 border border-gray-300 rounded-md bg-gray-50 xl:text-sm text-xs " value="{{ $periode->tgl_berakhir }}"/>
        </div>
        <div class="mb-4 col-span-12">
          ...
          <select id="template_berkas" name="template_berkas_id" placeholder="template berkas"
            ...
            @foreach ($template_berkas as $item)
                <option value="{{ $item->id }}" {{ ($item->id == $periode->template_berkas_id) ? 'selected' : "" }}>{{ $item->name }}</option>
            @endforeach
          </select>
        </div>
