...
@section('main')
    <section class="max-w-screen-lg min-h-screen mx-auto flex items-center pt-44 pb-20 px-4 lg:px-12 gap-4">
        <div class="w-full p-10 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
            ...
            <div class="mt-4">
                <form action="{{ route('kajur.itemberkas.store') }}" method="post" class="w-full">
                    @csrf
                    <div class="mb-4">
                            ...
                            @foreach ($data->itemBerkas as $item)
                                <div class="p-6 bg-slate-100 rounded-xl flex flex-col gap-y-4">
                                    ...
                                    <div class="detailContainer flex flex-col hidden">
                                        ...
                                        <div>
                                            <x-button_sm color="primary" class="edit-button"
                                                onclick="modalOpen(this, {{ $item }})">
                                                Aksi
                                            </x-button_sm>
                                        </div>
                                    </div>
                    ...
                    <div class="max-w-lg w-full p-6 bg-white rounded-xl">
                        ...
                        <div class="mb-4">
                            <form action="{{ route('kajur.item-management.update') }}" method="post" class="">
                                ...
                                <input type="hidden" name="id" id="id_modal" placeholder="Masukan Id Berkas"
                                    class="block w-full xl:p-4 p-3 text-gray-900 border border-gray-300 rounded-md bg-gray-50 xl:text-sm text-xs" />
    ...
    `;
        ...
        function modalOpen(value, data) {
            ...
            const id = value.parentElement.previousElementSibling.children[2].value;
            // console.log(value.parentElement.previousElementSibling.children)
            const modal = document.getElementById('modal');
            modal.classList.remove('hidden');
            modal.classList.add('flex')
            const inputStatis = document.getElementById('nama_berkas_statis');
            setModalInputValue(data);
            setModalIdValue(data.template_berkas_id);
            setModalIdDeleteValue(data.id);
        }
        ...
        function setModalInputValue(value) {
            ...
            inputModal.value = value.name;

            const inputId = document.getElementById('id_modal')
