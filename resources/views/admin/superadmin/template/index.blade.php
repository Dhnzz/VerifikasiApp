...
@section('main')
    <section class="max-w-screen-xl mx-auto min-h-screen flex flex-col pt-44 pb-20 px-4 lg:px-12 gap-4">
        <div class="flex justify-between lg:flex-row flex-col lg:items-center gap-y-4">
            ...
            <x-button_md color="primary" onclick="location.href='{{ route('kajur.template.create') }}';"
        ...
        @if (session('success'))
            <div class="bg-green-500 text-white p-4 rounded-md relative" id="notif">
                <p>{{ session('success') }}</p>
                <button class="absolute top-0 right-0 p-2 text-white" onclick="tutupNotifikasi()"><i class="fa fa-times"></i></button>
            </div>
        @elseif(session('error'))
            <div class="bg-red-500 text-white p-4 rounded-md relative" id="notif">
                <p>{{ session('error') }}</p>
                <button class="absolute top-0 right-0 p-2 text-white" onclick="tutupNotifikasi()"><i class="fa fa-times"></i></button>
            </div>
        @endif
                                ...
                                <div class="relative inline-block text-left">
                                    ...
                                    <div id="dropdownMenu{{ $item->id }}"
                                        ...
                                        <div class="py-1" role="none">
                                            <a href="{{ route('kajur.item-management.create', $item->id) }}"
                                            ...
                                            <a href="{{ route('kajur.template.edit', $item->id) }}"
                                            ...
                                            <form action="{{ route('kajur.template.destroy', $item->id) }}" method="POST"
