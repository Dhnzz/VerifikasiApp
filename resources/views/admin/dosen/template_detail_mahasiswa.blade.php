            ...
            <div class="flex flex-col">
                ...
                <p class="text-sm">{{ date('j M Y', strtotime($peserta->periode->tgl_mulai)) }} -
                    {{ date(
                        ...
                        class="text-slate-500">({{ round(\Carbon\Carbon::now()->diffInDays(\Carbon\Carbon::parse($peserta->periode->tgl_berakhir), false)) }}
                        hari )</span></p>
            </div>
        </div>
                        ...
                        <div class="detailContainer flex items-center justify-between hidden mt-4">
                            ...
                            <div id="modal"
                                ...
                                <div class="max-w-2xl w-full p-6 rounded-xl h-[80vh] overflow-hidden">
                                    ...
                                    @foreach ($mahasiswaBerkasId as $item)
                                        <embed src="{{ asset('storage/upload/'.$item->berkas) }}" type=""
                                            class="mt-4 w-full h-full">
                                    @endforeach
                                </div>
                            </div>
